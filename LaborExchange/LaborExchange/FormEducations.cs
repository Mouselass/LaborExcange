...
{
    public partial class FormEducations : Form
    {
        [Dependency]
        public new IUnityContainer Container { get; set; }
        private readonly EducationLogic logic;

        public FormEducations(EducationLogic logic)
        {
            InitializeComponent();
            this.logic = logic;
        }
        ...
        {
            var form = Container.Resolve<FormEducation>();
        ...
        private void ButtonDel_Click(object sender, EventArgs e)
        {
            if (dataGridView.SelectedRows.Count == 1)
            {
                if (MessageBox.Show("Удалить запись", "Вопрос", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    ...
                    try
                    {
                        logic.Delete(new EducationBindingModel { Id = id });
                    }
                    catch (Exception ex)
                    {
                        MessageBox.Show(ex.Message, "Ошибка", MessageBoxButtons.OK, MessageBoxIcon.Error);
